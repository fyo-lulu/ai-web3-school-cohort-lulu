...
建议学习顺序
先判断自己的已有基础：更偏 AI，还是更偏 Web3，还是两边都刚入门。
优先补短板：Web3 背景先补 AI 基础；AI 背景先补 Web3 基础。
至少完成两个方向的最小实践：一个 AI 工具任务，一个测试网 / 合约交互任务。
再做最小交叉实验，把 AI 输出、人工复核、钱包确认、链上执行和验证记录放到同一条流程里。
最后整理一份概念说明和过程记录，为后续项目方向讨论提供材料。
...
AI 输出为什么必须验证：1. 事实错误：自信编造不存在的信息，关键事实须外部核实；2. 引用错误：编造论文、链接、数据来源，不信任模型给的链接；3. 推理漂移：长上下文中逻辑链断裂，结论偏离前提，需分段验证；4. 执行越权：agent 做了超出授权范围的操作，须设置 guardrails 和 human-in-the-loop；5. 工具误用：调用了错误的工具或参数，须用 tracing 监控。
Agent 核心技术组件：状态管理，多节点共享读写同一 State；长期记忆，跨 session 存储与召回；MCP，LLM 与外部工具的统一连接协议；Skills，可复用的高层指令集，支持自动发现和动态生成；Tool Calling，模型输出结构化请求，框架执行后回传结果；Tracing，可视化 agent 执行链；Guardrails，输入输出验证规则，不符合则中止；Handoff，子任务完成后移交控制权；错误恢复，执行失败时的重试、回退和人工介入。
什么时候需要 agent？ 目标开放式、无法提前写死步骤；需要多工具协作；中间结果决定下一步策略；需跨会话记忆状态。更适合用简单方案的信号：一次性问答用 Prompt；流程固定用脚本；高合规要求用人工审核节点；数据确定性要求高用数据库查询。复杂度和风险越高，越要警惕过度 agent 化。
推荐材料 / 参考入口
...
如果你没有 OpenAI / Claude 会员：可以在 Z.ai / GLM 平台申请 API Key，并按需充值；本课程鼓励优先使用 GLM / Z.ai、Codex、Claude Code、Hermes 等工具路径；如没有 OpenAI / Claude 会员，可以在 Z.ai / GLM 平台申请 API Key，并按需充值。
...
没有 OpenAI / Claude 会员 → 可以在 Z.ai / GLM 平台申请 API Key，并按需充值；本课程鼓励优先使用 GLM / Z.ai、Codex、Claude Code、Hermes 等工具路径；如没有 OpenAI / Claude 会员，可以在 Z.ai / GLM 平台申请 API Key，并按需充值。
...
本周交付
Learning agent 与 GitHub repo 交付：Claude Code / Codex / Hermes 的配置记录、模型 / API 路径说明（OpenAI / Claude 会员，或 Z.ai / GLM API）、个人课程 repo 链接、至少一次 agent 协助学习或编码的日志，以及对应 commit / 截图 / README 证明。
...
OpenClaw 仓库：适合理解 agent 进入执行层带来的变化。
Hermes Agent Docs / Hermes 仓库：适合理解 agent、工具调用、skills 与长期执行的组织方式。
OpenAI Agents SDK：适合理解 handoff、guardrails、tracing 等 agent 原语。
...
Hardhat Getting Started / Foundry：用于最小合约开发、测试与部署。
viem / wagmi：适合有前端能力的学员做最小交互页。
OpenZeppelin Contracts：作为权限、token 与合约复用的安全基础设施入口。
...
Transformers Training：理解训练 / 微调基础流程。
PEFT 文档：理解参数高效微调。
Unsloth Docs：理解更贴近实操的轻量微调路径。
...
Intro to Ethereum / Blocks / Proof of Stake：补齐区块、共识与系统背景。
EVM / Gas and fees：理解执行环境与 Gas 机制。
